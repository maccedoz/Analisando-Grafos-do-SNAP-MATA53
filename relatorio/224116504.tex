...
%% article.
\begin{abstract}
  Este relatório apresenta uma análise estrutural e algorítmica detalhada da rede social  {Zachary's Karate Club} no contexto do framework Python  {Karate Club}. O documento está organizado em três partes principais. Na Parte I, foi realizada a caracterização estrutural obrigatória do grafo, computando e discutindo métricas de topologia fundamentais como densidade, diâmetro, comprimento médio dos caminhos, agrupamento local e distribuição de graus. Na Parte II, foi detalhada a implementação manual de oito algoritmos clássicos de grafos (BFS, DFS, Eulerianidade, Dijkstra, Bellman-Ford, Floyd-Warshall, Tarjan SCC e Kruskal MST), analisando suas complexidades teóricas e empíricas sob 50 rodadas de medições estatísticas. Na Parte III, foi conduzida uma análise estrutural avançada abordando a propriedade de pequeno mundo ( {small-world}), a lei de potência, a robustez estrutural sob falhas e ataques direcionados, a comparação com os modelos de Erdős-Rényi, Watts-Strogatz e Barabási-Albert, e correlacionando os resultados com a cisão histórica real do clube.
\end{abstract}
...
\begin{table*}[t]
  ...
  \begin{tabular}{llp{8cm}}
    ...
    Geração de pesos nas arestas & \textbf{Realizado} & Como a rede original é não-ponderada, foram atribuídos pesos sintéticos determinísticos via $w(u,v) = (u{+}v)\bmod 5 + 1$, gerando valores inteiros em $[1, 5]$. Isso viabiliza os algoritmos de caminho mínimo (Dijkstra, Bellman-Ford, Floyd-Warshall) e a MST (Kruskal). \\
    Mapeamento de atributos sociais & \textbf{Realizado} & Foi extraído o atributo \path{club} de cada nó do NetworkX, mapeando a facção final de cada membro ( {"Mr. Hi"} ou  {"Officer"}) para uso em visualizações e análise da cisão social. \\
...
\begin{enumerate}
    \item \textbf{Geração de Pesos nas Arestas:} Como a rede original é não-ponderada, a força do relacionamento foi simulada atribuindo pesos determinísticos não-negativos a cada aresta $(u, v)$ através da expressão:
    \begin{equation}
        w(u, v) = (u + v) \bmod 5 + 1
    \end{equation}
    ...
    \item \textbf{Mapeamento de Atributos Sociais:} O atributo \path{club} de cada nó foi extraído e registrado, indicando a facção final de cada membro após a cisão histórica do clube.

    \item \textbf{Conversão para Lista de Adjacência:} O grafo foi exportado no formato de lista de adjacência para o arquivo \path{data/karate_adj_list.txt}. Cada linha do arquivo segue o formato \path{nó: [vizinho1, vizinho2, ...]}, representando explicitamente a estrutura de vizinhança de cada vértice. Este formato é utilizado diretamente pelos algoritmos implementados do zero (BFS, DFS, Dijkstra, etc.).

    \item \textbf{Exportação para CSV Estruturado:} A rede tratada foi exportada para \path{data/karate_club_edges.csv}, com as colunas \path{source}, \path{target}, \path{weight}, \path{source_club} e \path{target_club}. Este arquivo serve como base de dados persistida e reproduzível do projeto.
\end{enumerate}

\section{Parte I - Análise Estrutural Obrigatória}
\label{sec:parte_i}

Nesta primeira etapa, foram computadas e discutidas as principais métricas de topologia de redes no grafo de Zachary. As métricas estruturais analisadas seguem as definições matemáticas clássicas:
\begin{itemize}
...
A topologia visualizada na Figura \ref{fig:karate_graph} (gerada através do script \path{scripts/gerar_graficos.py}) evidencia a divisão social clássica que deu origem à cisão do grupo de caratê.
...
\begin{enumerate}
    ...
    \item \textbf{Floyd-Warshall:} Registrou o maior tempo ($4958,726$ $\mu$s), sendo 700 vezes mais lento que os lineares. Isso justifica-se por sua estrutura de loops triplos aninhados O(V\textsuperscript{3}), cuja complexidade se sobressai mesmo para um pequeno número de nós.
\end{enumerate}
...
A distribuição empírica de graus \textbf{não} sugere uma lei de potência pura devido a severos efeitos de tamanho finito ( {finite-size effects}):
\begin{itemize}
...
Para avaliar a tolerância a falhas e ataques na rede de Zachary, foram simulados dois cenários distintos sob a remoção de $5\%$ dos vértices ($5\%$ de $34 \approx 2$ nós):
\begin{enumerate}
    \item \textbf{Remoção Aleatória (Falha):} Sorteio de 2 nós para exclusão. Foram simuladas $1000$ rodadas independentes para calcular o tamanho médio do maior componente conexo LCC restante e seu comprimento médio de caminhos.
...
Foi avaliada a aderência do grafo real aos três modelos canônicos de redes complexas:
\begin{enumerate}
...
\textbf{Conclusão:} O grafo real aproxima-se de um modelo \textbf{híbrido}. Trata-se de uma rede social típica, que conjuga o alto agrupamento local e os caminhos curtos do modelo \textbf{Watts-Strogatz} com a estrutura hierárquica baseada em hubs explicada pelo modelo \textbf{Barabási-Albert}.
...
Na caracterização de topologia (Parte I), foi comprovada a estrutura de mundo pequeno da rede, dotada de alto agrupamento local e baixo diâmetro, polarizada sob os dois nós de grau elevado representados pelo instrutor e pelo administrador do clube.

Na vertente algorítmica (Parte II), foram implementados com sucesso os algoritmos de busca e caminho mínimo, validando experimentalmente seu comportamento por meio de benchmarks estatísticos robustos. A concordância estrita entre as curvas teóricas e empíricas consolida a correlação prática direta com as análises conduzidas na disciplina.
