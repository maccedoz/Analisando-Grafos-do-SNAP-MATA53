...
Nesta seção, descrevo a implementação manual de algoritmos clássicos de grafos no diretório \path{algoritmos/} e a sua aplicação prática no grafo de Zachary. Para verificar preliminarmente o funcionamento correto de cada implementação, utilizei o script de demonstração \path{scripts/executar_algoritmos.py}.
...
Como o grafo possui 34 vertices, a árvore geradora mínima resultante contém exatamente 33 arestas, totalizando peso 51,0 com a minha distribuição de pesos.

\subsection{Análise Experimental de Desempenho}

Para analisar o tempo real de execução dos algoritmos implementados, executei o script \path{scripts/run_benchmarks.py} para realizar um benchmark de $n = 50$ rodadas. Em cada rodada, o algoritmo executa um número elevado de iterações internas. A média de tempo por iteração foi registrada para cada uma das 50 rodadas.
...
Para avaliar essa propriedade quantitativamente, utilizei o script \path{scripts/analise_estrutural.py} que calcula esses parâmetros do zero (sem bibliotecas de grafos prontas para as métricas), gera 100 grafos aleatórios equivalentes baseados no modelo de Erdős-Rényi que preserva a conexidade, e computa o índice de pequeno mundo de Humphries e Gurney ($\sigma_{sw}$):
\begin{equation}
